...
public class importSheet extends JFrame {
	...
	public ArrayList<BookInImportSheetPOJO> getListImportBook(){
		...
		for(int i = 0; i<listImportBookTable.getRowCount();i++ ){
			...
			BookInImportSheetPOJO book = new BookInImportSheetPOJO(idIPS, id_book, name_book, id_publisher, quantity, price);
	...
	public importSheet() {
		...
		addRowBtn.addActionListener(new ActionListener(){
			...
			public void actionPerformed(ActionEvent e) {
				...
				String[] row = {book.getId_book(), book.getName(), book.getId_publisher(), book.getQuantity()+"", book.getImport_price()+""};
				model.addRow(row);
